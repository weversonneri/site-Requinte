//compilar == sass --watch node_modules/bootstrap/scss:node_modules/bootstrap/compiler
//Problema = Ctrl+f5

@import url(https://fonts.googleapis.com/css?family=Roboto:300);


html {
	position: relative;
	min-height: 100%;
  }
  body {
	margin-bottom: auto;
  }

.image {
	
	width: 100%;
	height: auto;
  }


/*********** Login Page ***********/

.login-page > .form {
	border-radius: 5px;
}

.login-form input {
	border-radius: 5px;
}

.register-form input {
	border-radius: 5px;
}

.login-form  > button {
	border-radius: 5px;
}

.register-form > button {
	border-radius: 5px;
}
/********** Navbar ******************/
.navbar {
	min-height: 57px;
}

.navbar  ul > li {	
	color: white;
}

.navbar  ul > li > a {
	text-decoration: none;
	padding: 8px;
	color: white;
}

.navbar  ul > li > a:hover {
	padding: 8px;
	color: rgb(213, 161, 236);
}

.navbar-nav .dropdown {
	color: white;
	margin-right: 20px;
}

.navbar-nav .dropdown ul > li > a {
	color: black;	
}
.navbar-nav .dropdown ul > li > a:hover {
	color: rgbrgb(213, 161, 236);	
}
/********* Subnavbar ****************/

.subnavbar {
    	margin-bottom: 2.5em;
    }
    
.subnavbar-inner {
	height: 50px;
	background: #fff;
	border-bottom: 1px solid #d6d6d6;
}

.subnavbar .container > ul {
	display: inline-block;	
	height: 80px;
	padding: 0;
	margin: 0;
	
}

.subnavbar .container > ul > li {
	float: left;
	min-width: 90px;
	height: 50px;
	padding: 0;
	margin: 0;
	text-align: center;
	list-style: none;
	border-left: 1px solid #d9d9d9;
	
}

.subnavbar .container > ul > li > a {
	display: block;
	height: 100%;
	padding: 10px 15px;	
	font-size: 16px;
	font-weight: bold;
	color: #b2afaa;
}

.subnavbar .container > ul > li > a:hover {
	color: #888;
	text-decoration: none;
}

.subnavbar .container > ul > li > a > span {
	display: block;
	
}

.subnavbar .container > ul > li.active > a {
	
	border-bottom:5px solid #ff7f74;
	color: black;
}
/*********** Index ************/
.logo {
	max-width: 100px;
	max-height: 70px;
} 

#banner-index {
    background-image: url("../../../images/photo22.jpg");
    background-attachment: fixed;
	background-size: cover;
	padding-top: 100px;
	padding-bottom: 80px;
	justify-content: center;
	align-items: center;
	position: relative;
	
  }

.titulo-index {
	margin-top: 25px;
	text-align: center;
}

.titulo-index h1 {
	color: white;
	font-family: Cinzel Decorative;
	font-size: 60px ;
	font-weight: 900;
	text-shadow: 0px 1px, 1px 0px, 1px 1px white;
	letter-spacing: 2px;
}

.titulo-index h2 {
	color: #CC8A56;
	font-family: sans-serif;
	font-size: 30px ;
	font-weight: 200;
	margin-top: 40px;
	margin-bottom: 30px;
}

.titulo-index a{
	background-color: #CC8A56;
	border-color: #CC8A56;
	border-radius: 40px;
	font-family: Arial, Helvetica, sans-serif ;
	font-size: 15px;
	font-weight: 600;
	color: white;
	text-align: center;
	padding: 5px 40px 5px 40px;
}

.titulo-index a:hover{
	background-color: rgb(160, 108, 68);
	border-color: rgb(156, 104, 64);
	text-decoration: none;
	color: white;
}

.bar01 {
	border-top: 1px solid rgba(238, 238, 228, 0.445);
	margin: 80px 50px 0px 50px;
  } 

.info {
	font-family: cinzel;
	text-align: center;
	color: rgba(238, 238, 228, 0.795);
}

.info h3{
	font-size: 25px;
	font-weight: 100;
	margin-top: 20px;
	margin-bottom: 20px;
}

.info h4{
	font-size: 20px;
	font-weight: 100;
}


#salon {
	background-image: url("../../../images/fundo2.jpg");
    background-attachment: fixed;
	background-size: cover;
	padding-top: 100px;
	padding-bottom: 100px;
	justify-content: center;
	align-items: center;
	position: relative;
}

.salon-info {
	text-align: center;
	margin-bottom: 50px;
}

.salon-info h3 {
	font-family: Cinzel;
	font-size: 35px;
	font-weight: 600;
	color: #73024B;
	text-align: center;
}

.bar02 {
	border-top: 1px solid black;
	margin: 20px 222px 0px 222px;
  } 

.salon-info p {
	font-family: sans-serif;
	font-size: 18px;
	font-weight: 500;
	color: black;
	text-align: center;
	margin: 20px 50px 50px 50px;
}

.salon-info a {
	background-color: #01080A;
	border-color: #01080A;
	border-radius: 40px;
	font-family: Arial, Helvetica, sans-serif ;
	font-size: 15px;
	font-weight: 600;
	color: white;
	padding: 7px 20px 7px 20px;
}

.salon-info a:hover{
	background-color: #73024B;
	border-color: #73024B;
	text-decoration: none;
	color: white;
}

#salon img {
	align-content: center;
	max-width: 600px;
	max-height: 600px;
	width: 100%;
	height: auto;
}


.trabalhos {
	background: #000 url(../../../images/background.jpg);
	background-size: cover;
}

.trabalhos h3 {
	color: #FFF;
	padding-top: 50px;
}

.trabalhos img {
	width: 200px;
	height: 200px;
}

#demo {
	height:100%;
	position:relative;
	overflow:hidden;
  }
  
  
  .green{
	background-color:#6fb936;
  }
		  .thumb{
			  margin-bottom: 30px;
		  }
		  
		  .page-top{
			  margin-top:85px;
		  }
  
	 
  img.zoom {
	  width: 100%;
	  height: 200px;
	  border-radius:5px;
	  object-fit:cover;
	  -webkit-transition: all .3s ease-in-out;
	  -moz-transition: all .3s ease-in-out;
	  -o-transition: all .3s ease-in-out;
	  -ms-transition: all .3s ease-in-out;
  }
		  
   
  .transition {
	  -webkit-transform: scale(1.2); 
	  -moz-transform: scale(1.2);
	  -o-transform: scale(1.2);
	  transform: scale(1.2);
  }

/********* Agendamento **********/
.titulo {
	margin-top: 25px;
	text-align: center;
}

.services {
	width: 80%;
	margin: auto auto auto auto;
	padding: 20px;
	margin-top: 40px;
}


/******** Footer ***********/

.top-bar {
	padding: 10px 0;
	background: rgb(34, 34, 34);
	border-bottom: 1px solid rgb(59, 59, 59);
	line-height: 28px;
  }
  
  .social{
	text-align: center;
  }
  
  .social-share{
   display: inline-block;
   list-style: none;
   padding: 0;
   margin: 0;
  }
  
  ul.social-share li {
	display: inline-block;
  }
  
  ul.social-share li a {
	display: inline-block;
	color: #fff;
	background: #404040;
	width: 25px;
	height: 25px;
	line-height: 25px;
	text-align: center;
	border-radius: 2px;
  }
  
  ul.social-share li a:hover {
	background: rgb(213, 161, 236);
	color: #fff;
  }

.footer {
	bottom: 0;
	width: 100%;
	height: 60px;
    background-color: rgb(34, 34, 34);
	padding: 10px;
}

.footer-inner {
	padding: 15px 0;
	font-size: 12px;
	background: rgb(34, 34, 34);
	color: #999;
}

.footer a {
	color: #999;
}

.footer a:hover {
	color: #FFF;
	text-decoration: none;
}

/************** Others *******************/
.admTitle {
	color: rgba(0, 0, 0, 0.61);
	text-align: center;
	padding-bottom: 20px;
}

#tabela {
	margin-bottom: 50px;
}

